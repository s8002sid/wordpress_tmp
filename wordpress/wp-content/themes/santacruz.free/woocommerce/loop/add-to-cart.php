...
if ( ! is_shop_enabled() || ! $product->is_purchasable() ) {
    $add_to_cart = '';
    $out_of_stock = '';
...
<?php } else { ?>
    ...
    switch ( $product->product_type ) {
        case "variable" :
            ...
            $add_to_cart = sprintf('<a href="%s" rel="nofollow" class="view-options button" title="%s">%s</a>', $link, $label, $label);
            break;
        case "grouped" :
...
<?php
    $show_add_cart=($product->is_purchasable() || $product->product_type == 'external');
?>
