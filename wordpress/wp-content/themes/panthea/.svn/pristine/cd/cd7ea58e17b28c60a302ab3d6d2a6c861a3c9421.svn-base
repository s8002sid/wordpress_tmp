...
class search_mini extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		<form action="<?php echo home_url( '/' ); ?>" method="get" class="search_mini">
			<input type="text" name="s" id="search_mini" value="<?php the_search_query(); ?>" placeholder="<?php _e( 'search', 'yit' ); ?>" />
			<input type="hidden" name="post_type" value="<?php echo $search_type ?>" />
			<input type="submit" value="Search" id="mini-search-submit" class="button" /> <!-- Default Style Button -->
		</form>
...
<?php		echo $after_widget;
	...
	function form( $instance ) {
        ...
        _e('The widget cannot be used in this sidebar', 'yit');
