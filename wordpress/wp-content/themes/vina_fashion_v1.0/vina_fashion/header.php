<?php
/**
 * @package Vina Fashion
 * @author VinaThemes.biz http://www.VinaThemes.biz
 * @copyright Copyright (c) 2013 VinaThemes.biz
 * @license http://www.gnu.org/licenses/gpl-2.0.html GNU/GPLv2 or Later
*/
?><!DOCTYPE html>
<!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7"  <?php language_attributes(); ?>> <![endif]-->
<!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8"  <?php language_attributes(); ?>> <![endif]-->
<!--[if IE 8]>         <html class="no-js lt-ie9"  <?php language_attributes(); ?>> <![endif]-->
<!--[if gt IE 8]><!--> <html class="no-js" <?php language_attributes(); ?>> <!--<![endif]-->

<head>
    <link rel="profile" href="http://gmpg.org/xfn/11" />
    <link rel="pingback" href="<?php bloginfo( 'pingback_url' ) ?>" />
    <meta charset="<?php bloginfo( "charset" ) ?>" />
    <meta name="viewport" content="width=device-width" />
    <title><?php Helix::theTitle(); ?></title>
    <?php
		Helix::Header(); //wp_head() included
    ?>
</head>

<body id="sp-wrapper" <?php Helix::bodyClass("hfeed bg"); ?>>
<div id="sp-top-wrapper">
	<div class="container">
		<div class="row-fluid">
			<div id="sp-menu" class="span7">
				<?php helix::addFeatures('menu'); ?>
			</div>
			<div class="span5 menu-top-right">
				<?php Helix::addWidgets(array('menutopright')); ?>
			</div>
		</div>
    </div>
</div>
<header id="sp-header-wrapper" role="banner">
    <div class="container">
		<div class="row-fluid">
			<div class="span4 logo-header">
				<?php helix::addFeatures('logo'); ?>
			</div>
			<div class="span4 visible-desktop">
				<?php get_search_form(); ?>
			</div>
            <div class="span4 shopcart">
				<?php Helix::addWidgets(array('shoppingcart')); ?>
			</div>
		</div>
    </div>
</header>
<?php if ( Helix::hasWidgets( array( 'menumain') ) ) : ?>
<div id="sp-menu-wrapper">
	<div class="container">
		<?php /*?><?php Helix::addWidgets(array('menumain')); ?><?php */?>
        <?php helix::addFeatures('menuproduct'); ?>
    </div>
</div>
<?php endif; ?>
<?php if ( is_home() || is_front_page() ) : ?>
	<div id="sp-feature-wrapper">
		<?php Helix::addWidgets(array('feature')); ?>
	</div>
<?php endif; ?>
<?php if ( Helix::hasWidgets( array( 'user1', 'user2', 'user3', 'user4' ) ) ) : ?>
	<section id="sp-users-wrapper">
        <div class="container">
            <?php Helix::addWidgets( array( 'user1', 'user2', 'user3', 'user4' ) ); ?>
        </div>
	</section>
<?php endif; ?>

<section id="sp-main" role="main">

<div id="sp-main-body-wrapper">	
	<div class="container" id="content">
	
		<div class="row-fluid">
		
			<?php if (Helix::hasWidgets(array('left'))) : ?>
				<aside id="leftsidebar" class="span<?php echo Helix::Param('left_sidebar_width'); ?>">
					<?php Helix::addWidgets(array('left')); ?>
				</aside>
			<?php endif; ?>
			
			<div id="contentsidebar" class="span<?php echo Helix::mainWidth(); ?>">
            	<?php if ( Helix::hasWidgets( array( 'slidecategory') ) ) : ?>
                    <div id="sp-slide-category">
                        <?php Helix::addWidgets(array('slidecategory')); ?>
                    </div>
                <?php endif; ?>
            	<?php Helix::addFeatures('breadcrumbs'); ?>
            	<?php if ( Helix::hasWidgets( array( 'bestseller') ) ) : ?>
                    <div id="sp-bestseller-wrapper">
                        <?php Helix::addWidgets(array('bestseller')); ?>
                    </div>
                <?php endif; ?>
                <?php if ( Helix::hasWidgets( array( 'onlineshipping ') ) ) : ?>
                    <div id="sp-online-shipping">
                        <?php Helix::addWidgets(array('onlineshipping ')); ?>
                    </div>
                <?php endif; ?>		