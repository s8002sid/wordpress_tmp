...
class Custom_WooCommerce_Widget_Best_Sellers extends WP_Widget {
	 ...
	 * @return void
	 */
	function Custom_WooCommerce_Widget_Best_Sellers() {
		...
		if ( $r->have_posts() ) {
			...
			while ( $r->have_posts()) {
				...
				if ( 0 == $woocommerce_loop['loop'] % $woocommerce_loop['columns'] )
					$classes[] = 'last';
				?>
				<li id="product-items" <?php post_class( $classes ) ?>">
					<?php do_action( 'woocommerce_before_shop_loop_item' ); ?>
					<?php	
					echo '<div class="img-pro"><a href="' . get_permalink() . '">' . ( has_post_thumbnail() ? get_the_post_thumbnail( $r->post->ID, 'shop_thumbnail' ) : woocommerce_placeholder_img( 'shop_thumbnail' ) ) . ' 
                        </a></div> 
                    	<div class="info-pro">
						<div class="info-pro-wrapper">
                        <p class="title-pro"><a href="' . get_permalink() . '">' . get_the_title() . '</a></p>
